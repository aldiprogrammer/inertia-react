...
export default function Penjualan({ penjualan }) {
    const { data, setData, post, delete: destroy, processing, error } = useForm({
                                ...
                                <tbody>
                                    {penjualan.map((item, index) => (
                                        <tr>
                                            ...
                                            <td>{Number(item.uang).toLocaleString('id-ID')}</td>
                                            <td>{Number(item.kembalian).toLocaleString('id-ID')}</td>
                                            ...
                                            <td>
                                                <div className='flex gap-2'>
                                                    ...
                                                    <button className='btn btn-success btn-sm' onClick={() => editopenModal(item.id, item.kode, item.meja)}>Detail</button>
                                                </div>
                                            </td>
                                        </tr>
